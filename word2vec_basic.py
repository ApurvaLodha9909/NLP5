...
def generate_batch(data, batch_size, num_skips, skip_window):
  ...
  #skip_windows*2+1 = window size
  windowsize = (skip_window*2)+1
  ...
  data_buffer = collections.deque(maxlen = windowsize)
  #deque is used for fast push and pop operations
  ...
  #keep moving the window i.e. keep on updating the data buffer and use it to update the batch and labels
  for currwindow in range(0,batch_size // num_skips,1):
      list = [skip_window]
      contextWordIndex = skip_window

      for offset in range(0,num_skips,1):
          while contextWordIndex in list:
              contextWordIndex = random.randint(0, windowsize - 1)

          batch[currwindow * num_skips + offset] = data_buffer[skip_window]
          #print (data_buffer[skip_window])
          labels[currwindow * num_skips + offset, 0] = data_buffer[contextWordIndex]
          #print (data_buffer[contextWordIndex])
          list.append(contextWordIndex)
...
if __name__ == '__main__':
  ...
  with tf.Session(graph=graph) as sess:
    ...
    embeddings = train(sess, model, data, dictionary, batch_size, num_skips, skip_window,
                        max_num_steps, checkpoint_step, loss_model)
    endtime = time.time()
    print("Time: {}s".format((time.time()-stime)))
